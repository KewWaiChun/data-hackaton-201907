...
The data we are going to use is a retail dataset. This data set is an mock of a given retailer company might collect. The dataset consists of 5,000 rows and 21 columns.
...
Raw data almost always comes dirty, meaning it consists of missing values, errors, misspellings, duplicate records, etc. Using a dirty dataset will produce inaccurate results when performing analysis. 
...
The Gender column consists of the values M, F and male. You will need to standardize the values by changing:
* M -> Male
* F -> Female
* male -> Male
...
Follow the same procedure as above to load the second data set into GIANT (i.e. US_states_statistics.csv). You will now see two data sets in "Connect to Source(s)" page.
...
You might have noticed that the key column is now repeated twice in the merged data set. Go ahead and edit the merged data source to hide the duplicated column. Don't forget to save the changes.
